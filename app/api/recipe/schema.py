...
class RecipeIngridientGetter(GetterDict):
    def get(self, key: Any, default: Any = None) -> Any:
        if key == 'ingredients':
            return [
                association.ingredient.name for association in self._obj.ingredients
            ]
...
class UploadRecipeStep(BaseModel):
    ...
    def validate_first_step(cls, duration: timedelta, values):
        if duration < timedelta(seconds=60):
            raise ValueError(
                "Duration must be at least 1m " f"not {duration.total_seconds()}s."
            )
        return duration
...
RecipeListOrder: TypeAlias = (
    Literal['duration'] | Literal['-duration'] | Literal['rating'] | Literal['-rating']
)
